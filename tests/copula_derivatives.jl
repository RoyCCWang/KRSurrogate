...
# CDF = xx->collect( CDF_array[d](xx[1:d]) for d = 1:D )
#
# dG_ND = aa ->vec(FiniteDiff.finite_difference_jacobian(CDF, aa))
# d2G_ND_mat = xx->FiniteDiff.finite_difference_jacobian(dG_ND, xx)

d2G_x0_ND = collect( FiniteDiff.finite_difference_hessian(CDF_array[d], x0[1:d]) for d = 1:D )
println("ND: d2G(x0):")
display(d2G_x0_ND)
println()
...
dT_FD = xx->FiniteDiff.finite_difference_jacobian(Tmap, xx)
d2T_FD = xx->FiniteDiff.finite_difference_jacobian(dT_FD, xx)

d2T_x0_FD = d2T_FD(x0)

D_x = D
D_y = D
d2T_x0_FD_array = reshape(d2T_x0_FD,D_y,D_x,D_x)

A = collect( d2Tmap_ND[d](x0) for d = 1:D )

# this is troubling. two FD routiens not returning the same derivative.
